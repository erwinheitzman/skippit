...
const { gitHandler } = require('../utils/SourceControlHandler');
const configExists = existsSync(join(__dirname, '../../config.json'));
const config = configExists ? require('../../config') : null;
const packageJson = require('../../package.json');
const version = packageJson.version;
const supportedResultFormats = ['json', 'xml'];
const supportedTestFormats = ['e2e-spec.js', 'spec.js', 'js'];
const configQuestions = [
    {
        ...
        validate: answer => existsSync(answer)
    },
    {
        type: 'checkbox',
        name: 'resultFormats',
        message: 'Which reporter formats do you want to use?',
        choices: supportedResultFormats,
        default: 'xml',
        validate: answers => answers
            .every(format => supportedResultFormats.includes(format))
    },
    {
        type: 'input',
        name: 'testPath',
    ...
    },
    {
        ...
        choices: supportedTestFormats,
        default: 'e2e-spec.js',
        validate: answers => answers
            .every(format => supportedTestFormats.includes(format))
    },
    {
        type: 'input',
        name: 'remotePath',
        ...
        validate: answer => answer && typeof answer === 'string'
    },
    {
        type: 'input',
        name: 'repoPath',
...
function statusCommand () {
    ...
    function createLogFile (data) {
        ...
        writeFile('./' + fileName, data, 'utf8', (e) => {
            if (e) {
                throw e;
            }
        });
        ...
        const logStatus = (settings = config) => {
            ...
            simpleGit
                ...
                .diffSummary((err, status) => {
                    const { insertions } = status;
                    const suffix = insertions === 1 ? 'test' : 'tests';

                    if (err) {
                        throw new Error(err);
                    }
                    console.info(`Disabled ${insertions} ${suffix}`);
                })
                .diff((err, status) => {
                    if (err) {
                        throw err;
